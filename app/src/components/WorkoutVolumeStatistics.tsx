...
function processWorkoutStats(stats: WorkoutStats[]): WorkoutVolumeData[] {
  ...
  const workoutGroups = stats.reduce<Record<string, WorkoutStats[]>>(
    (acc, stat) => {
  ...
  return Object.entries(workoutGroups)
    .map(([workoutName, workoutStats]) => {
      // Sort by date
...
export function WorkoutVolumeStatistics() {
  const {
    ...
    isLoading,
    error,
  } = useQuery(() => WorkoutsAPI.getWorkoutStats());
  const [selectedWorkout, setSelectedWorkout] = useState<string>("");

  const workoutVolumeData = useMemo(() => {
    if (!workoutStatsData?.stats) return [];
    return processWorkoutStats(
      workoutStatsData.stats.filter((stat) => stat.totalWeight > 0),
    );
  }, [workoutStatsData]);

  const selectedWorkoutData = useMemo(() => {
    if (!selectedWorkout) return null;
    return workoutVolumeData.find(
      (data) => data.workoutName === selectedWorkout,
    );
        ...
        <CardContent className="pt-6">
          <div className="text-red-800">
            <h3 className="font-semibold mb-2">
              Error loading workout volume data
            </h3>
            <p>{error}</p>
          </div>
        </CardContent>
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Track your training volume over time by workout type
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center text-gray-500 py-12">
            ...
            <p>
              Start recording workouts with exercises to see your volume
              progression!
            </p>
          </div>
        </CardContent>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Workout Volume Analysis</CardTitle>
          <CardDescription>
            Select a workout to see how your training volume has changed over
            time
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {selectedWorkoutData && (
        <Card>
          <CardHeader>
            <CardTitle>
              Volume Progression - {selectedWorkoutData.workoutName}
            </CardTitle>
            <CardDescription>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-semibold">
                    {selectedWorkoutData.totalSessions}
                  </div>
                </div>
                <div>
                  <span className="text-gray-600">Total Volume:</span>
                  <div className="font-semibold text-green-700">
                    {Math.round(selectedWorkoutData.totalVolume)}kg
                  </div>
                </div>
                <div>
                  <span className="text-gray-600">Average Volume:</span>
                  <div className="font-semibold text-blue-700">
                    {Math.round(selectedWorkoutData.avgVolumePerSession)}kg
                  </div>
                </div>
                <div>
                  ...
                  <div className="font-semibold text-purple-700">
                    {selectedWorkoutData.progress.length > 1
                      ? selectedWorkoutData.progress[
                          selectedWorkoutData.progress.length - 1
                        ].totalWeight >
                        selectedWorkoutData.progress[0].totalWeight
                        ? "↗️ Improving"
                        : "↘️ Declining"
                      : "📊 Single Session"}
                  </div>
        ...
        {workoutVolumeData.map((data) => (
          ...
          >
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                {data.totalSessions} sessions recorded
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-gray-500">
                    {format(parseISO(data.firstSessionDate), "MMM d")} -{" "}
                    {format(parseISO(data.lastSessionDate), "MMM d")}
                  </span>
                </div>
              </div>
            </CardContent>
